module.exports = function(crowi, app) {
  ...
  actions.existsPage = function(req,res) {
    var path = req.body.path;
    Page.findPageByPath(path)
    .then(function () {
      return res.json(ApiResponse.success({
        exists:true,
        message: 'ページ(' + path + ')は既に存在しています'
      }));
    }).catch(function (err) {
      if(err){
        ...
        return res.json(ApiResponse.success({exists:false}));
      }
    });
  };
  ...
  actions.pageEdit = function(req, res) {
    ...
    .then(function(data) {
      ...
      if (data) {
        ...
        return Page.updatePage(data, body, req.user, {grant: grant,canEdit:canEdit });
      } else {
        // new page
        updateOrCreate = 'create';
        return Page.create(path, body, req.user, {grant: grant,canEdit:canEdit});
      }
    }).then(function(data) {
  ...
  api.joinable = function(req, res) {
    var page_id = req.body.page_id,
        joinable = req.body.joinable;

    Page.findPageByIdAndGrantedUser(page_id, req.user)
    .then(function(pageData) {
      return pageData.updateJoinAble(joinable);
    }).then(function(data) {
      var result = {joinable: data.joinAble};
  ...
  api.join = function(req, res) {
    ...
    .then(function(data) {
      return data.pushMember(req.user);
    }).then(function(data) {
      var result = {};
      result.page = data;
  ...
  api.leave = function(req, res) {
    ...
    Page.findPageById(page_id)
    .then(function(data) {
      return data.removeMember(req.user);
    }).then(function(data) {
      var result = {};
      result.page = data;
      result.user = req.user;
