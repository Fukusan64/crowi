module.exports = function(crowi) {
  ...
  pageSchema = new mongoose.Schema({
    ...
    canEdit: { type: Number, default: EDIT_PUBLIC, index: true  },
    ...
    joinedUsers: [ { type: ObjectId, ref: 'User', index: true } ],
    extended: {
      type: String,
      default: '{}',
      get: function(data) {
  ...
  pageSchema.methods.isEdit = function(){
    if(this.canEdit === EDIT_PUBLIC){
      return true;
    }
    return false;
  };
  ...
  pageSchema.methods.isJoined = function(userData) {
    var page = this;
    if (!page.joinAble) {
      return false;
    }
    if (!page.joinedUsers || !Array.isArray(page.joinedUsers)) {
      page.joinedUsers = [];
    }
    return page.joinedUsers.indexOf(userData._id) != -1;
  };
  ...
  pageSchema.methods.isEditFor = function(userData){
    if(this.isEdit() || this.isCreator(userData)){
      return true;
    }
    return false;
  };
  ...
  pageSchema.methods.updateJoinAble = function(joinable) {
    var page = this;
    return new Promise(function(resolve, reject) {
      page.joinAble = joinable;
      return page.save (function (err, data) {
        if (err) {
          return reject(err);
        }
        return resolve(data);
  ...
  pageSchema.methods.pushMember = function(userData) {
    var page = this;

    return new Promise(function(resolve, reject) {
      if (!page.joinAble) {
        return reject(new Error("This page isn't joinable."));
      }
      ...
      if (page.joinedUsers.indexOf(userData._id) != -1) {
        return reject(new Error("You are already joined."));
      }
      page.joinedUsers.push(userData._id);
  ...
  pageSchema.methods.removeMember = function(userData) {
    var page = this;
    ...
    return new Promise(function(resolve, reject) {
      ...
      var beforeCount = page.joinedUsers.length;
      page.joinedUsers.pull(userData._id);
      if (beforeCount != page.joinedUsers.length) {
        page.save (function (err, data) {
          if (err) {
            return reject(err);
          }
        ...
        return reject(new Error('joined user not dated'));
      }
    });
  };
  ...
  pageSchema.statics.updateCanedit = function(page, canEdit, userData) {
    return new Promise(function(resolve, reject) {
  ...
  pageSchema.statics.create = function(path, body, user, options) {
    var Page = this
      ...
      , canEdit = options.canEdit || EDIT_PUBLIC
    ...
    return new Promise(function(resolve, reject) {
      ...
      .then(function(revision) {
        if (grant != pageData.grant) {
          ...
          return Page.updateCanedit(pageData, canEdit, user).then(function(data) {
            resolve(data);
            pageEvent.emit('update', data, user);
          });
      }).catch(function(err) {
